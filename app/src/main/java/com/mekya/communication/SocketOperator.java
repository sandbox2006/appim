...
{
	private static final String AUTHENTICATION_SERVER_ADDRESS = "http://208.91.114.182:8999/server/index.php";
	...
	private Socket getSocket(InetAddress addr, int portNo) 
	{
		...
		if (sockets.containsKey(addr) == true) 
		{
			...
			if  ( socket.isConnected() == false || socket.isInputShutdown() == true ||
				  socket.isOutputShutdown() == true || socket.getPort() != portNo )
			{			
